...
func TestStore(t *testing.T) {
	...
	ar := &domain.Article{
		...
		CreatedAt: now,
		UpdatedAt: now,
		Author:    domain.Author{ID: 1, Name: "arupaka"},
	}
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	...
	prep.ExpectExec().WithArgs(ar.Title, ar.Author.ID, ar.CreatedAt, ar.UpdatedAt).
...
func TestGetByTitle(t *testing.T) {
	...
	query := "SELECT idtitle ,content, author_id,updated_at,created_at FROM article WHERE title = \\?"
